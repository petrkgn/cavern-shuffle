...
План разработки хранится в файле `PLAN.md` в корне проекта.

**Когда обращаться:**
Если в запросе упоминается **план разработки**, **план**, **задачи** или конкретный пункт плана (например, «[BUG] Сломана логика перетасовки»), необходимо обратиться к файлу `PLAN.md` для получения актуальной информации о задачах.

**Команды для работы с планом:**
- «покажи план разработки» — вывести содержимое `PLAN.md`
- «возьми задачу из плана» — выбрать следующую задачу из плана
- «отметь задачу [номер] выполненной» — обновить чекбокс в `PLAN.md`

**Важно:** Отмечать задачу выполненной (`[x]`) только после подтверждения от пользователя.

**Процесс работы с задачами:**
1. При упоминании плана — прочитать `CHANGELOG.md` (если существует) для сверки последних изменений, затем прочитать `PLAN.md`
2. Выбрать задачу согласно приоритетам или указанию пользователя
3. Выполнить задачу
4. Дождаться подтверждения от пользователя
5. После подтверждения — отметить чекбокс `[x]` в `PLAN.md`
6. Изменения в `PLAN.md` коммитить вместе с кодом

**Зачем:**
- Отслеживание прогресса разработки
- Приоритизация задач (критичные баги → баги механик → оптимизация)
...
**Формат имени коммита (Conventional Commits):**
```
...
1. **Тип** (на английском):
   ...
   - `feat` — новая функция
   - `docs` — документация (PLAN.md, CHANGELOG.md, README.md)
   - `chore` — вспомогательные изменения (настройка, конфигурация)
   - `refactor` — рефакторинг без изменения поведения
   - `perf` — улучшение производительности

2. **Сущность** (на английском, в скобках):
   - `game` — игровая логика, механики
   - `card` — карты, их поведение
   - `ui` — интерфейс, GUI
   - `item` — предметы (Short Rest, Potion, Treasure)
   - `enemy` — враги, боссы
   - `party` — отряд, уровни, сила
   - `docs` — документация проекта
   - `config` — конфигурационные файлы

3. **Описание** (на английском, с большой буквы):
   - Краткое описание **что сделано в контексте игры**
   - **Без** внутренней реализации (названий функций, файлов)
   - Фокус на поведении для игрока

**Примеры:**
```
fix(item): Fixed Short Rest shuffle animation not playing correctly
feat(card): Added dragon transformation visual effect
docs(docs): Added development plan with task tracking
fix(party): Treasure card now properly increases party power level
chore(config): Updated game resolution settings
```

**Важно:** Все действия с git (коммит, пуш) выполняются **только после подтверждения пользователя**.

**Процесс работы:**
1. Выполняю изменения в коде
2. Показываю пользователю результат
3. После команды «закоммить» — готовлю черновик коммита с правильным именем
4. Показываю черновик на подтверждение
5. После подтверждения — выполняю `git commit`
6. **`git push` выполняется пользователем вручную**
