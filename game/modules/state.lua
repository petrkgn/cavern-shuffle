...
function M.init()
	...
	M.data.piles.explore = Pile.new({
		id = "explore",
		pos = vmath.vector3(CONST.COLUMN_X[1], CONST.ROW_Y[1], 0),
		layout = Pile.LAYOUT_STACK,
		placeholder_sprite = "card_back",
	})
	M.data.piles.explore.can_use = true

	-- Inventory (создаются с персональными плейсхолдерами)
	local inventory_placeholders =
		{ "item_placeholder_1", "item_placeholder_2", "item_placeholder_3", "item_placeholder_4" }
	for i = 1, 4 do
		local id = "inventory_" .. i
		M.data.piles[id] = Pile.new({
			id = id,
			pos = vmath.vector3(CONST.COLUMN_X[1], CONST.ROW_Y[i + 1], 0),
			layout = Pile.LAYOUT_STACK,
			placeholder_sprite = inventory_placeholders[i],
		})
	end

	-- Talon (БЕЗ плейсхолдера)
	M.data.piles.talon = Pile.new({
		id = "talon",
		pos = vmath.vector3(CONST.COLUMN_X[2], CONST.ROW_Y[1], 0),
		layout = Pile.LAYOUT_STACK_OFFSET,
	})
	-- Discard (БЕЗ плейсхолдера)
	M.data.piles.discard = Pile.new({
		id = "discard",
		pos = vmath.vector3(CONST.COLUMN_X[3] + 50, CONST.ROW_Y[1], 0),
		layout = Pile.LAYOUT_STACK,
	})
	...
	for i, suit in ipairs(party_suits) do
		...
		M.data.piles[id] = Pile.new({
			id = id,
			pos = vmath.vector3(CONST.COLUMN_X[i + 4], CONST.ROW_Y[1], 0),
			layout = Pile.LAYOUT_STACK,
			suit = suit,
			placeholder_sprite = "party_placeholder_" .. suit,
		})
	end
	...
	for i = 1, 7 do
		...
		M.data.piles[id] = Pile.new({
			id = id,
			pos = vmath.vector3(CONST.COLUMN_X[i + 1], CONST.ROW_Y[2], 0),
			layout = Pile.LAYOUT_CASCADE,
		})
	end
