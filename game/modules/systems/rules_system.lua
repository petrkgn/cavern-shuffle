...
local LESS_TABLE = { ["K"]="Q", ["Q"]="J", ["J"]="10", ["10"]="09", ["09"]="08", ["08"]="07", ["07"]="06", ["06"]="05", ["05"]="04", ["04"]="03", ["03"]="02", ["02"]="A" }

function M.is_lawful(suit) return suit == "hearts" or suit == "diamonds" end
function M.is_chaotic(suit) return suit == "spades" or suit == "clubs" end
...
function M.is_valid_dungeon_move(dragged_card_data, target_pile_card_data)
	...
	local alignments_alternate = (M.is_lawful(dragged_card.suit) and M.is_chaotic(target_card.suit)) or (M.is_chaotic(dragged_card.suit) and M.is_lawful(target_card.suit))
	if not alignments_alternate then return false end
	...
	if dragged_card.type == GameConfig.TYPE_ADVENTURER then
		if #target_pile.cards == 0 then
			...
			if target_card.name == "Treasure" then
				...
				if dragged_card.suit ~= target_card.suit then return false end
...
function M.is_valid_inventory_move(dragged_card_data, target_pile)
	...
	if dragged_card.type ~= GameConfig.TYPE_ITEM then return false end
	if #target_pile.cards > 0 then return false end
	return true
end

function M.is_drag_allowed(pile, card_index)
	if not string.find(pile.id, "dungeon") then return true end
...
function M.can_clear_obstacle(state, obstacle_card_info)
	...
	print(string.format("RULE CHECK: Obstacle '%s'. Difficulty: %d, Adventurer Level (w/buff): %d", obstacle_card_info.name, difficulty, adventurer_level))
...
function M.can_defeat_enemy(state, enemy_card_info)
	...
	print(string.format("RULE CHECK: Enemy '%s'. Required Power: %d, Party Power (w/buff): %d", enemy_card_info.name, enemy_card_info.power, party_power))
...
function M.is_valid_item_use(state, item_card_info, target_pile)
	...
	elseif item_card_info.name == "Treasure" then
		return string.find(target_pile.id, "party") ~= nil and #target_pile.cards > 0 and CardUtils.get_card_info(target_pile.cards[#target_pile.cards]).value < 10
		...
		for _, card_index in ipairs(source.card_indexes) do
			...
			if card_info.type == GameConfig.TYPE_ADVENTURER then
				...
				for i = 1, 7 do
					...
					if dest_pile ~= source.pile and M.is_drag_allowed(dest_pile) then
						if M.is_valid_dungeon_move(card_to_move, dest_pile.cards[#dest_pile.cards]) then
							local hint = string.format("Можно переместить %s %s из '%s' в столбец %d.", card_info.suit, card_info.face, source_name, i)
				...
				for i = 1, 4 do
					...
					if M.is_valid_party_move(card_to_move, dest_pile) then
						local hint = string.format("Можно повысить уровень героя %s картой %s.", card_info.suit, card_info.face)
						table.insert(possible_moves, hint)
					end
				end
			...
			elseif card_info.type == GameConfig.TYPE_OBSTACLE then
				if M.can_clear_obstacle(state, card_info) then
					table.insert(possible_moves, "Можно пройти препятствие: " .. card_info.name .. "!")
				end
			elseif card_info.type == GameConfig.TYPE_ITEM then
				...
				for i = 1, 4 do
					...
					if M.is_valid_inventory_move(card_to_move, dest_pile) then
						local hint = string.format("Можно поместить предмет '%s' в инвентарь.", card_info.name)
						table.insert(possible_moves, hint)
