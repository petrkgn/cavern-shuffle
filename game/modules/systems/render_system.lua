...
function M.move_card(card, to_pile, to_index, delay, on_complete, easing, from_pile)
	...
	if not card.go_id then
		...
		card.go_id = factory.create("#card_factory", start_pos, nil, {
			face = hash(card.face),
			suit = hash(card.suit),
...
function M.redraw_pile(pile)
	...
	for i, card in ipairs(pile.cards) do
		...
		if card.go_id then
			...
			card.go_id = factory.create("#card_factory", pos, nil, {
				face = hash(card.face),
				suit = hash(card.suit),
				hidden = card.is_hidden and hash("hidden") or hash(""),
			}, CONST.CARD_SCALE)
		end
	end
end
...
function M.update_explore_pile_sprite(explore_pile)
	if not explore_pile or not explore_pile.placeholder_go then
		return
	end
