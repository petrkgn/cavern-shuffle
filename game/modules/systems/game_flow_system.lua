...
local function create_placeholder_card(state, target_pile, is_dungeon_pile)
	...
	if not top_card_data then return false end
	...
	if is_dungeon_pile then
		...
		if next_value == 11 then next_face = "J" end -- Исправление для перехода 10 -> J
	end

	if not next_face then return false end

	local next_suit = is_dungeon_pile
		and (RulesSystem.is_lawful(top_card_info.suit) and "spades" or "hearts")
...
function M.restore_item_from_joker(state, joker_card, target_pile)
	...
	if not restored_key then return end
...
function M.deal(state)
	...
	local piles_to_animate = {"dungeon_1", "inventory_1", "dungeon_2", "dungeon_3", "dungeon_4", "dungeon_5", "dungeon_6", "dungeon_7"}
					...
					on_complete = function()
						...
						for i = 1, 7 do
							...
							if not p.placeholder_go then
								...
								"#card_factory", vmath.vector3(p.pos.x, p.pos.y, -0.1), nil,
								{ card = hash("card_empty") }, CONST.CARD_SCALE
							)
						end
					end
				end
	...
	timer.delay(0.3, false, function()
		if #explore_pile.cards > 0 then
			...
			for i, card in ipairs(cards_to_add_to_talon) do
				...
				RenderSystem.move_card(card, talon_pile, #talon_pile.cards, (i-1)*0.05, on_complete, easing_type, explore_pile)
			end
...
timer.delay(1, false, function()
	...
	if not clicked_card_data.go_id then return end
...
function M.use_short_rest(state, item_card_data, source_slot_id)
    ...
    timer.delay(0.4, false, function()
        ...
        for i = total_cards, 1, -1 do
            ...
            RenderSystem.move_card(card, explore_pile, #cards_to_shuffle, (total_cards - i) * 0.03, nil, go.EASING_INOUTSINE, discard_pile)
        end
        ...
        timer.delay(total_cards * 0.02 + 0.4, false, function()
            ...
            for i = total_cards, 1, -1 do
                ...
                table.insert(discard_pile.cards, card)
                RenderSystem.move_card(card, discard_pile, #discard_pile.cards, (total_cards - i) * 0.03, nil, go.EASING_INOUTSINE, explore_pile)
            end
...
function M.check_for_duplicate(state, card_to_check, source_pile)
	if not card_to_check then return false end
	...
	if state.created_cards[card_key] then
		...
		timer.delay(delay, false, function()
			...
			if not card_to_check.go_id or not go.exists(card_to_check.go_id) then return end
			...
			RenderSystem.animate_to_graveyard(card_to_check, function()
				...
				if #source_pile.cards > 0 then
					...
					if new_top_card.is_hidden then
						new_top_card.is_hidden = false
						if new_top_card.go_id then msg.post(new_top_card.go_id, "reveal") end
...
for _, card in ipairs(pile.cards) do
	if card.go_id then table.insert(all_cards, card) end
end
end
...
if not state.is_won then return end
state.is_restarting = true
